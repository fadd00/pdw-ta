...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setFormData({ name: '', description: '', price: '', color: 'amber', image: '' })
        setShowForm(false)
              ...
              <div key={product.id} className="border border-gray-200 rounded-lg p-4">
                <div className="text-center mb-4">
                  {product.image ? (
                    ...
                    <div className={`w-16 h-24 bg-gradient-to-b ${getColorGradient(product.color)} rounded-t-full mx-auto mb-2 opacity-70`}></div>
                  )}
                </div>
                <h3 className="text-lg font-medium text-amber-900 mb-2">{product.name}</h3>
                <p className="text-gray-600 text-sm mb-2">{product.description}</p>
                <p className="text-amber-800 font-medium">${product.price}</p>
              </div>
            ))}
          </div>
        </div>
